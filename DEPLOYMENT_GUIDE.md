...
# Получить Chat ID: php artisan telegram:chat-id или @userinfobot
...
### 10. Настройка Telegram уведомлений (рекомендуется)

**Важно:** Telegram уведомления позволяют отслеживать все действия бота в реальном времени.

**Полные инструкции:** См. файл `TELEGRAM_SETUP.md` в корне проекта.
...
2. **Получение Chat ID:**
   ...
   - Используйте команду: `php artisan telegram:chat-id`
...
4. **Тестирование:**
   ...
   Команда отправит 6 тестовых уведомлений в Telegram.

**Что будут отправлять уведомления:**
- 🚀 Запуск команды `bots:run`
- 🔵 DRY RUN BUY/SELL (тестовые сделки)
- 🟢 REAL BUY/SELL (реальные сделки)
- ✅ ORDER FILLED (ордер выполнен)
- ⏸️ NO ACTION TAKEN (сигнал HOLD)
- ⚠️ TRADE SKIPPED (пропуск сделки)
- ❌ ERROR (ошибки)
- 💰 POSITION CLOSED (закрытие позиции с PnL)

**Примечание:** Если Telegram не настроен, бот будет работать, но уведомления отправляться не будут (ошибки будут только в логах).
...
# 3. Тестирование Telegram уведомлений (если настроено)
php artisan telegram:test
# Должно отправить 6 тестовых уведомлений в Telegram

# 4. Запуск в dry_run режиме
