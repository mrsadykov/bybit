...
## Одна команда для всех ботов

**Бэктест spot + futures + btc-quote в одном запуске:**  
`php artisan strategy:backtest-all-bots [--period=500] [--exchange=okx] [--output=файл.json]`

Запускает бэктест для всех спотовых ботов, всех фьючерсных (OKX) и всех BTC-quote (OKX), выводит краткие результаты по каждому и в конце — сводку по типам (ТОП по доходности). При `--output` сохраняет один JSON: `{ "spot": [...], "futures": [...], "btc_quote": [...] }`.

---

## Отдельные команды по типам
