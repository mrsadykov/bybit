...
## 1. Важно про RSI на споте

При **TRADING_CONSERVATIVE_RSI=true** пороги RSI для **спот-ботов** берутся не из БД (38/62, 45/55), а **подменяются на 35/65** глобально. То есть результат `strategy:apply-optimize` для спота в этом режиме не используется — действуют 35/65. Если нужны именно 38/62 и 45/55, поставь `TRADING_CONSERVATIVE_RSI=false`.
