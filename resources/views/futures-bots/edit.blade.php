<x-app-layout>
                ...
                <div class="p-6 text-gray-900">
                    <form id="futures-bot-edit-form" action="{{ route('futures-bots.update', $bot) }}" method="POST">
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('futures-bots.show', $bot) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-900 font-bold py-2 px-4 rounded border border-gray-500">
                                {{ __('futures.cancel') }}
                            </a>
    ...
    <div class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-lg py-4 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto flex flex-wrap items-center justify-end gap-3">
            <a href="{{ route('futures-bots.show', $bot) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-900 font-bold py-2 px-4 rounded border border-gray-500">{{ __('futures.cancel') }}</a>
            <button type="submit" form="futures-bot-edit-form" class="bg-blue-200 hover:bg-blue-400 text-blue-900 font-bold py-2 px-4 rounded border border-blue-600">{{ __('futures.update') }}</button>
            <form action="{{ route('futures-bots.destroy', $bot) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('futures.confirm_delete') }}');">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-200 hover:bg-red-400 text-red-900 font-bold py-2 px-4 rounded border border-red-600">{{ __('futures.delete') }}</button>
            </form>
        </div>
    </div>
</x-app-layout>
