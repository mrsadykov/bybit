<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center gap-3 min-w-0">
            ...
            <form action="{{ route('bots.reset-risk-baseline', $bot) }}" method="POST" class="shrink-0" onsubmit="return confirm('{{ __('bots.reset_risk_baseline_confirm') }}');">
                @csrf
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form id="bot-edit-form" action="{{ route('bots.update', $bot) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="max_daily_loss_usdt" id="max_daily_loss_usdt"
                                value="{{ old('max_daily_loss_usdt', $bot->max_daily_loss_usdt) }}" step="0.01" min="0"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="max_drawdown_percent" id="max_drawdown_percent"
                                value="{{ old('max_drawdown_percent', $bot->max_drawdown_percent) }}" step="0.01" min="0" max="100"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="use_macd_filter" value="1"
                                    {{ old('use_macd_filter', $bot->use_macd_filter) ? 'checked' : '' }}
